...
export const imageUpdate = async (imageId: string, payload: IUpdateImage, accessToken: string) => {
  const res = await axios.put(`${apiAddress}/api/images/${imageId}`, payload, {
    headers: { Authorization: `Bearer ${accessToken}` },
  });
  return res.data;
};

export const imageDelete = async (imageId: string, accessToken: string) => {
  const res = await axios.delete(`${apiAddress}/api/images/${imageId}`, {
    headers: { Authorization: `Bearer ${accessToken}` },
  });
  return res;
};
